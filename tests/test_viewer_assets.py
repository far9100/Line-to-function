...
def test_the_page_draws_every_curve_desmos_gets():
    """The canvas previews Desmos, which cannot fill: an area is its outline plus the curves inside it."""
    source = (serve.VIEWER_DIR / "viewer.js").read_text(encoding="utf-8")
    assert "ctx.fill(" not in source.replace("ctx.fill();", "")  # only the selection dots use fill()
    assert 'tags.includes("fill")' not in source  # nothing is hidden from the canvas
