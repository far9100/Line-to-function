...
line2func traces every line in a drawing (or photo) into cubic parametric
...
- Everything runs offline. The viewer needs no build step and no internet.
...
docs/           # third_party.md (licenses of third-party code and weights)
tests/          # pytest suite
```
...
- **Third-party:** the pretrained line-art network is *Informative Drawings*
  (Chan, Durand & Isola, CVPR 2022), MIT license; its generator code is ported
  in `line2func/lineart_model.py` and its weights are downloaded, never bundled.
  Details, hashes and license texts are in
  [`docs/third_party.md`](docs/third_party.md). Only third-party resources that
  allow commercial use are used.
- **Related work:**
  - *Deep Vectorization of Technical Drawings* (ECCV 2020, [arXiv:2003.05471](https://arxiv.org/abs/2003.05471)) inspired the model design.
  - Curve fitting follows Schneider's algorithm (*Graphics Gems*, 1990).
  - [zenith600/image-to-desmos](https://github.com/zenith600/image-to-desmos) is a similar in-browser tool.
...
configs/        # 訓練設定檔
docs/           # third_party.md（第三方程式碼與權重的授權）
tests/          # pytest 測試
```
...
- **本專案授權：** [GNU General Public License v3.0 或更新版本](LICENSE)（GPL-3.0-or-later）。
- **第三方資源：** 預訓練線稿網路是 *Informative Drawings*（Chan、Durand、Isola，CVPR 2022），採 MIT 授權；它的生成器程式碼移植在 `line2func/lineart_model.py`，權重則是另外下載，從不隨專案附帶。細節、雜湊值與授權全文見 [`docs/third_party.md`](docs/third_party.md)。本專案只使用允許商業用途的第三方資源。
- **相關研究與專案：**
  - *Deep Vectorization of Technical Drawings*（ECCV 2020，[arXiv:2003.05471](https://arxiv.org/abs/2003.05471)）啟發了本專案的模型設計。
  - 曲線擬合採用 Schneider 演算法（*Graphics Gems*，1990）。
  - [zenith600/image-to-desmos](https://github.com/zenith600/image-to-desmos) 是在瀏覽器中運作的類似工具。
